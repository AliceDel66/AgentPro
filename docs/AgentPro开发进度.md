...
- 完成功能：
  ...
  - 需求访谈页取消右侧 Agent 工作流面板，需求状态改为顶部状态条；AgentSpec 生成/审批、Runner 策略选择、真实开发启动/取消、事件查看和评审报告生成改为对话流内的顺序 workflow
  - workflow 增加逐步门禁：有待确认反问时不展示后续开发步骤，AgentSpec 未确认不展示 Runner，开发未启动不展示监控，任务未完成不展示自动评审
