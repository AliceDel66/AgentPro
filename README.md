...
- 多轮需求访谈、主动追问、批量问题回答、AgentSpec 草案生成与审批。
- 严格对话式 workflow：用户始终停留在需求访谈对话界面，每一步回答或审批后才出现下一步。
...
AgentPro 的主路径不是多个独立页面之间跳转，而是一条留在对话界面的线性 workflow。用户提出需求后，顶部只展示当前需求状态、成熟度、已回答数和待回答数；正文继续保持聊天流。系统在当前步骤完成前，不展示或启用后续步骤，避免用户误以为可以跳过需求澄清。
...
- 顶部状态条只展示状态，不承载跳步操作。
- 需求澄清是 workflow 的第 0 步；存在待回答问题时，不展示 AgentSpec、Runner 和评审操作。
- 后续步骤一次只展示当前可执行步骤：AgentSpec 草案、开发方式、运行监控、自动评审、评审结果。
- “提交回答”必须先持久化用户答案并快速返回，模型整理不能阻塞提交成功。
- followup 问题是 question-answer 对，不是“适用/不适用”检查项；多个问题必须在当前批次全部填写后统一提交。
...
AGENTPRO_REQUIREMENT_AI_TIMEOUT_SECONDS=12
